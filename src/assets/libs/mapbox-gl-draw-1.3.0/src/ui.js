...
export default function (ctx) {
  ...
  function createControlButton(id, options = {}) {
    const binderDOM = document.getElementById(options.binderID)

    const button = binderDOM || document.createElement("button");
  ...
  function addButtons() {
    ...
    const binderIDMap = ctx.options.binderIDMap

    const callbackMap = ctx.options.callbackMap
    ...
    if (controls['circle']) {
      const binderID = binderIDMap[3]

      const dom = document.getElementById(binderID)

      dom.onclick = callbackMap.draw_circle.bind(this) 
    }

    if (controls['rectangle']) {
      const binderID = binderIDMap[4]

      const dom = document.getElementById(binderID)

      dom.onclick = callbackMap.draw_rectangle.bind(this) 
    }
